...
namespace SelGate
{
    class SelGate
    {
        ...
        private string ClientPort = "7100";
        private string SrvHost = "127.0.0.1";      // DBServer的地址
        private string SrvPort = "5100";
        ...
        private void SendServerPackThread()
        {
            while (true)
            {
                while (SrvSeassion != null)     // 确保连上了服务器取得session才会发包
                {
                    // 取出一条待发送包，并判断是否有效
